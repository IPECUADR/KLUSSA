...
try {
    ...
    if (!$stmtExiste->fetch()) {
        echo json_encode([
            'err' => true,
            'mensaje' => 'El paciente que intenta actualizar no existe'
        ], JSON_UNESCAPED_UNICODE);
        exit;
    }
    ...
    $validaciones = [
        [
            'sql' => 'SELECT PK_prs FROM persona WHERE hcl_num = :valor AND PK_prs != :PK_prs LIMIT 1',
            'valor' => $hcl_num,
            'mensaje' => 'Número de historia clínica repetido'
        ],
        [
            'sql' => 'SELECT PK_prs FROM persona WHERE ci_prs = :valor AND PK_prs != :PK_prs LIMIT 1',
            'valor' => $ci_prs,
    ...
    if ($email_prs !== '') {
        $validaciones[] = [
            'sql' => 'SELECT PK_prs FROM persona WHERE email_prs = :valor AND PK_prs != :PK_prs LIMIT 1',
            'valor' => $email_prs,
    ...
    if ($num_cel_prs !== '') {
        $validaciones[] = [
            'sql' => 'SELECT PK_prs FROM persona WHERE num_cel_prs = :valor AND PK_prs != :PK_prs LIMIT 1',
            'valor' => $num_cel_prs,
    ...
    $sqlUpdate = "
        ...
        SET
            hcl_num = :hcl_num,
            nombre_prs = :nombre_prs,
            apellido_ps = :apellido_ps,
            ci_prs = :ci_prs,
            email_prs = :email_prs,
