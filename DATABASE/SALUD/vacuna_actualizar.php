...
try {
    ...
    $vacunaActual = $stmtExiste->fetch();

    if (!$vacunaActual) {
        echo json_encode([
            'err' => true,
            'mensaje' => 'La vacuna que intenta actualizar no existe'
        ], JSON_UNESCAPED_UNICODE);
        exit;
    }
    ...
    if ($stmtDuplicado->fetch()) {
        echo json_encode([
            'err' => true,
            'mensaje' => 'Ya existe otra vacuna con este tipo y dosis para el paciente seleccionado'
        ], JSON_UNESCAPED_UNICODE);
        exit;
    }

    $evidenciaFinal = $vacunaActual['evidencia_vc'] ?? '';
    ...
    if (isset($_FILES['archivo_evidencia_vc']) && $_FILES['archivo_evidencia_vc']['error'] !== UPLOAD_ERR_NO_FILE) {
        ...
        $nombreSeguro = 'vacuna_' . $PK_vc . '_' . date('Ymd_His') . '_' . bin2hex(random_bytes(4)) . '.' . $extension;
        ...
        $evidenciaFinal = 'RESULTADOS/SALUD/VACUNAS/' . $nombreSeguro;
    }
    ...
    echo json_encode([
        'err' => true,
        'mensaje' => 'Error al actualizar vacuna'
    ], JSON_UNESCAPED_UNICODE);
}